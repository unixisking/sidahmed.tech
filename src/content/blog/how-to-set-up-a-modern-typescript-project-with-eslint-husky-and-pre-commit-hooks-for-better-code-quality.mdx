...
publishedDate: 07 June 2024
image:
  - path: hello
  - alt: image alt
---
#  How to Set Up a Simple TypeScript Project with ESLint, Husky, and Pre-Commit Hooks
...
$ npm init -y
```
Will also install `typescript` as a dev dependency and initialize our project.
...
- **strict**: enabled strict mode which is a restricted variant of Javascript
- **esModuleInterop**: enables es module interoperability with CommonJS which allows us to import CommonJS modules using ES module syntax.
...
- **outDir**: specifies the directory where the transpiled Javascript will be located
...
We can update the target option as well as the module option to the latest versions by setting the following:
...
```
You can also use **esnext** as a target which corresponds to the latest version of ECMAScript, **es2023** is used here for stability. The recommended option for the module option is **NodeNext** which supports both cjs and esm modules.
...
```
Next, lets create the config file for eslint
```js
// eslint.config.js
// @ts-check
...
Lets add this command to our package.json file
...
Lets install husky and configure it
```bash
...
Lets edit this file and replace npm run test with **npm run lint**
...
Lets install lint-staged and configure it so that we can run our linter only on staged files
```bash
...
```
Lets modify our pre-commit file and add the lint-staged command instead
```bash
# .husky/pre-commit
npx lint-staged
```
...
We can also take this a step further by seting up a Github action that runs eslint in your CI pipeline
